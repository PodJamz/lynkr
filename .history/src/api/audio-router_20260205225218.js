...
/**
 * Remove undefined/null values so ACE-Step uses its own defaults
 */
...
/**
 * Create a task in ACE-Step - forwards ALL supported parameters
 *
 * ACE-Step 1.5 parameters: prompt, lyrics, audio_duration, audio_format,
 * bpm, key_scale, time_signature, vocal_language, reference_audio_path,
 * audio_cover_strength, task_type, src_audio_path, instruction,
 * repainting_start, repainting_end, inference_steps, guidance_scale,
 * seed, use_random_seed, batch_size, model, lm_model_path, thinking,
 * sample_mode, sample_query, use_format, lm_temperature, lm_top_p,
 * lm_top_k, lm_repetition_penalty, shift, infer_method, use_tiled_decode
 */
async function createAceStepTask(request) {
  ...
  logger.info({
    ...
    taskType: request.task_type || "text2music",
    thinking: request.thinking || false,
  }, "Creating ACE-Step task");

  const hasSeed = request.seed !== undefined && request.seed !== null && request.seed !== -1;

  const body = filterDefined({
    // Core
    prompt: request.prompt,
    lyrics: request.lyrics,
    audio_duration: request.duration || 30,
    audio_format: request.format || "mp3",

    // Music attributes
    bpm: request.bpm,
    key_scale: request.key || request.key_scale || request.keyScale,
    time_signature: request.time_signature || request.timeSignature,
    vocal_language: request.vocal_language || request.vocalLanguage || "en",

    // Reference / Edit audio
    reference_audio_path: request.reference_audio || request.referenceAudio,
    audio_cover_strength: request.reference_strength || request.referenceStrength,
    task_type: request.task_type || request.taskType || "text2music",
    src_audio_path: request.src_audio || request.srcAudio,
    instruction: request.instruction,
    repainting_start: request.repainting_start || request.repaintingStart,
    repainting_end: request.repainting_end || request.repaintingEnd,

    // Generation control
    inference_steps: request.inference_steps || request.inferenceSteps,
    guidance_scale: request.guidance_scale || request.guidanceScale,
    seed: hasSeed ? request.seed : undefined,
    use_random_seed: !hasSeed,
    batch_size: request.batch_size || request.batchSize,

    // Model selection
    model: request.model || "acestep-v15-turbo",
    lm_model_path: request.lm_model || request.lmModel || "acestep-5Hz-lm-1.7B",

    // LM parameters
    thinking: request.thinking,
    sample_mode: request.sample_mode || request.sampleMode,
    sample_query: request.sample_query || request.sampleQuery,
    use_format: request.use_format || request.useFormat,
    lm_temperature: request.lm_temperature || request.lmTemperature,
    lm_top_p: request.lm_top_p || request.lmTopP,
    lm_top_k: request.lm_top_k || request.lmTopK,
    lm_repetition_penalty: request.lm_repetition_penalty || request.lmRepetitionPenalty,

    // Advanced DiT
    shift: request.shift,
    infer_method: request.infer_method || request.inferMethod,
...
function formatAudioResponse(aceStepResult, taskId) {
  ...
  return {
    ...
    metadata: {
      ...
      vocal_language: metas.vocal_language || metas.language,
    },
...
router.post("/generate", async (req, res, next) => {
  try {
    ...
    // Validate request - all other params are optional and forwarded as-is
    ...
    // Forward all params to ACE-Step (createAceStepTask handles normalization)
